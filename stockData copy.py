...
class own_MLP(nn.Module):
    ...
    def __init__(self):
        ...
        self._layers = LAYERS
        self.dp = DP
        self.inputLayer = nn.Linear(SEQ_LEN_PAST * NUM_INPUTS, self._layers[0])

        self.hiddenLayer = []
        for i in range(len(self._layers) - 1):
            layer = nn.Linear(self._layers[i], self._layers[i + 1])
            self.hiddenLayer.append(layer)

        # Output layer
        self.outputLayer = nn.Linear(self._layers[len(self._layers)-1], SEQ_LEN_FUTURE * NUM_INPUTS)
    ...
    def preprocess_input(self, x):
        return x.view(-1, SEQ_LEN_PAST * NUM_INPUTS)
